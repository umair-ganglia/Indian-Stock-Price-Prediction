...
# 1️⃣ fetch (whatever your real method is called)
data_ok = getattr(predictor, "fetch_data")()
if data_ok is None:
    st.error("❌ Could not download price data. Check symbol or internet.")
    return

# 2️⃣ prepare (only if such a method exists)
if hasattr(predictor, "prepare_data"):
    predictor.prepare_data()
elif hasattr(predictor, "preprocess_data"):
    predictor.preprocess_data()        # adapt to YOUR real name
# else: nothing to prepare – continue
