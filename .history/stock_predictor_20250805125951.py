...
class IndianStockPredictor:
    ...
    def add_technical_indicators(self, df):
        ...
        return df
    # Add this method to your IndianStockPredictor class after add_technical_indicators

def add_advanced_features(self, df):
    """Add advanced market features and external indicators"""
    
    # Market microstructure features
    df['Price_Efficiency'] = abs(df['Close'] - df['Open']) / (df['High'] - df['Low'] + 1e-8)
    df['Intraday_Return'] = safe_divide((df['Close'] - df['Open']), df['Open'], 0)
    df['Overnight_Return'] = safe_divide((df['Open'] - df['Close'].shift(1)), df['Close'].shift(1), 0)
    
    # Advanced volatility measures
    df['GARCH_Volatility'] = df['Price_Change'].ewm(span=21).std() * np.sqrt(252)
    df['Realized_Volatility'] = df['Price_Change'].rolling(21).apply(lambda x: np.sqrt(np.sum(x**2)))
    
    # Market regime detection
    df['Trend_Strength'] = abs(df['Close'].rolling(20).apply(lambda x: safe_divide((x.iloc[-1] - x.iloc[0]), x.iloc[0], 0)))
    df['Market_Regime'] = np.where(df['Trend_Strength'] > 0.1, 1, 
                                   np.where(df['Trend_Strength'] < -0.1, -1, 0))
    
    # Fractal indicators
    df['Fractal_Dimension'] = df['Close'].rolling(14).apply(self._calculate_fractal_dimension)
    
    # Sentiment indicators (using volume and price action)
    df['Volume_Price_Trend'] = df['Volume'] * df['Price_Change']
    df['Accumulation_Distribution'] = safe_divide(((df['Close'] - df['Low']) - (df['High'] - df['Close'])), (df['High'] - df['Low']), 0) * df['Volume']
    df['AD_Line'] = df['Accumulation_Distribution'].cumsum()
    
    # Price momentum features
    df['Price_Acceleration'] = df['Price_Change'].diff()
    df['Volume_Weighted_Price'] = safe_divide((df['Volume'] * df['Close']).rolling(10).sum(), df['Volume'].rolling(10).sum(), df['Close'])
...
def _calculate_fractal_dimension(self, series):
    ...
    try:
        ...
        tau = [np.sqrt(np.std(np.subtract(series.iloc[lag:].values, series.iloc[:-lag].values))) for lag in lags]
        poly = np.polyfit(np.log(lags), np.log(tau), 1)
        return poly[0] * 2.0
    except:
        return 0.5

# Fix your LSTM training method by replacing the broken one:
def train_lstm(self, sequence_length=None, epochs=None, batch_size=None):
    ...
    try:
        ...
        # Try to recover by clearing session and retrying once
        try:
            K.clear_session()
            import time
            time.sleep(2)
            print("🔄 Retrying LSTM training...")
        except:
            print("❌ LSTM retry also failed")
        return None

# Update your prepare_data method to include advanced features:
def prepare_data(self):
    ...
    # Add advanced features  <-- Add this line
    df = self.add_advanced_features(df)
    
    # Rest of your existing prepare_data method remains the same...
    ...
    def build_lstm_model(self, sequence_length=None):
        ...
        return model
    # Add this to your stock_predictor.py in the train_lstm method
def train_lstm(self, sequence_length=None, epochs=None, batch_size=None):
    """Train LSTM model with improved error handling"""
    if not TENSORFLOW_AVAILABLE:
        print("⚠️ TensorFlow not available. Skipping LSTM training.")
    ...
    try:
        ...
        # Rest of your LSTM training code...
        # (Keep the existing code but add better exception handling)
