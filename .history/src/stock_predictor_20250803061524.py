...
# Configuration constants (since config.py might not exist)
class Config:
    # Data directories
    DATA_DIR = "data/"
    ...
    # Model parameters
    MIN_DATA_POINTS = 30  # Reduced from 100 to handle shorter periods
    TEST_SIZE = 0.2
    LSTM_SEQUENCE_LENGTH = 10  # Reduced from default
    LSTM_EPOCHS = 50
    LSTM_BATCH_SIZE = 32
    PREDICTION_DAYS = 30
...
class IndianStockPredictor:
    ...
    def add_technical_indicators(self, df):
        ...
        if len(df) >= config.RSI_PERIOD:
            delta = df['Close'].diff()
            gain = (delta.where(delta > 0, 0)).rolling(window=config.RSI_PERIOD, min_periods=max(1, config.RSI_PERIOD//2)).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(window=config.RSI_PERIOD, min_periods=max(1, config.RSI_PERIOD//2)).mean()
        ...
        df['High_Close_ratio'] = df['High'] / (df['Close'] + 1e-8)
        df['Low_Close_ratio'] = df['Low'] / (df['Close'] + 1e-8)
        ...
        df['ROC'] = df['Close'].pct_change(periods=momentum_period).fillna(0) * 100  # Rate of Change
        ...
        if len(df) >= 20:
            ...
            tp_ma = typical_price.rolling(window=20, min_periods=10).mean()
            mad = typical_price.rolling(window=20, min_periods=10).apply(
                lambda x: np.abs(x - x.mean()).mean() if len(x) > 0 else 0
            )
            df['CCI'] = (typical_price - tp_ma) / (0.015 * mad + 1e-8)
    ...
    def prepare_data(self):
        ...
        for col in numeric_columns:
            if df[col].isna().any():
                if 'lag' in col.lower():
                    # For lag features, use the mean of the original column
                    base_col = col.split('_lag_')[0]
                    if base_col in df.columns:
                        df[col] = df[col].fillna(df[base_col].mean())
                    else:
                        df[col] = df[col].fillna(df[col].mean())
                else:
                    df[col] = df[col].fillna(df[col].mean())
    ...
    def build_lstm_model(self, sequence_length=None):
        ...
        model = Sequential([
            LSTM(32, return_sequences=True, input_shape=(sequence_length, 1)),  # Reduced complexity
            Dropout(0.2),
    ...
    def evaluate_models(self):
        ...
        for model_name, pred_data in self.predictions.items():
            try:
                ...
                mape = np.mean(np.abs((pred_data['test_actual'] - pred_data['test_pred']) / (pred_data['test_actual'] + 1e-8))) * 100
    ...
    def plot_technical_indicators(self):
        ...
        df = self.processed_data.tail(min(200, len(self.processed_data))).copy()  # Last 200 days or available
